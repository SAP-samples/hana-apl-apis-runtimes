...
DO BEGIN
    ...
    :apply_config.insert(('APL/ApplyWithDeviation', 'true',null));

    call "apl_apply_debrief_example"(:header, :model,  :apply_config, 'USER_APL','ADULT01_SORTED', 'USER_APL','APPLY_OUT',out_log,out_sum,out_debrief_metric, out_debrief_property);
...
select * from "SAP_PA_APL"."sap.pa.apl.debrief.report::Deviation_ByVariable"(APPLY_DEBRIEF_PROPERTY, APPLY_DEBRIEF_METRIC);
select * from "SAP_PA_APL"."sap.pa.apl.debrief.report::Deviation_ByCategory"(APPLY_DEBRIEF_PROPERTY, APPLY_DEBRIEF_METRIC);
select * from "SAP_PA_APL"."sap.pa.apl.debrief.report::Deviation_CategoryFrequencies"(APPLY_DEBRIEF_PROPERTY, APPLY_DEBRIEF_METRIC); 
select * from "SAP_PA_APL"."sap.pa.apl.debrief.report::Deviation_ByGroup"(APPLY_DEBRIEF_PROPERTY, APPLY_DEBRIEF_METRIC);
select * from "SAP_PA_APL"."sap.pa.apl.debrief.report::Deviation_GroupFrequencies"(APPLY_DEBRIEF_PROPERTY, APPLY_DEBRIEF_METRIC); 
