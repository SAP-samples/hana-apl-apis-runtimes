...
<#
.SYNOPSIS
    SAP HANA APL Installation & Runtime Checker (Windows standalone PowerShell version)

.DESCRIPTION
    This is a STANDALONE script: check_apl.sql and check_create_CHECK_APL_user.sql
    are embedded at the bottom of this file and extracted at runtime.

    Connects to a SAP HANA instance and runs the APL check SQL script.

.PARAMETER HanaHost
    TENANT DB host and port (default: hana:30015)

.PARAMETER User
    TENANT DB system user (default: SYSTEM)

.PARAMETER Password
    TENANT DB system user password (default: Manager1)

.PARAMETER Format
    Output format: md (Markdown), raw, etc. (default: md)

.PARAMETER SignalError
    Signal error in output: on or off (default: off)

.PARAMETER OutputFile
    File path to store output. Use 'stdout' for console output (default: hana.md.txt)

.PARAMETER UseSsl
    Use SSL for connection: on or off (default: on)

.PARAMETER CheckAplPassword
    Password for the temporary CHECK_APL user (default: Password01Password01)

.PARAMETER ShowCmdOnly
    Show the final hdbsql command and exit without running it

.EXAMPLE
    .\check_apl.ps1 -HanaHost hana:30015 -User SYSTEM -Password MyPassword -OutputFile hana.md.txt

.EXAMPLE
    .\check_apl.ps1 -HanaHost hana:30015 -User SYSTEM -Password MyPassword -OutputFile hana.md.txt -ExtraArgs @('-e', '-ssltrustcert')
#>
...
param(
    ...
    [string]$OutputFile        = '',
    [string]$UseSsl            = '',
    [string]$CheckAplPassword  = '',
    [switch]$ShowCmdOnly,
    [string[]]$ExtraArgs       = @()
)

$DefaultHost             = if ($env:SAP_HOSTNAME) { $env:SAP_HOSTNAME } else { 'hana:30015' }
$DefaultUser             = 'SYSTEM'
$DefaultPassword         = 'Manager1'
$DefaultFormat           = 'md'
$DefaultSignalError      = 'off'
$DefaultUseSsl           = 'on'
$DefaultCheckAplPassword = 'Password01Password01'
$DefaultOutputFile       = 'hana.md.txt'

# --------------------------------------------------------------------------
# Extract an embedded SQL block from this script file.
# Blocks are delimited by lines starting with: #___BLOCK_START___ / #___BLOCK_END___
# --------------------------------------------------------------------------
function Extract-Block {
    param([string]$Tag)
    $scriptLines = [System.IO.File]::ReadAllLines($PSCommandPath)
    $startMarker = "#___${Tag}_START___"
    $endMarker   = "#___${Tag}_END___"
    $startIdx = -1
    $endIdx   = -1
    for ($i = 0; $i -lt $scriptLines.Count; $i++) {
        if ($scriptLines[$i].TrimEnd() -eq $startMarker) { $startIdx = $i }
        if ($scriptLines[$i].TrimEnd() -eq $endMarker)   { $endIdx   = $i }
    }
    if ($startIdx -lt 0 -or $endIdx -lt 0) {
        throw "Embedded SQL block '$Tag' not found in $PSCommandPath"
    }
    return $scriptLines[($startIdx + 1)..($endIdx - 1)] -join "`n"
}

# Extract SQL to temp files
$sqlScript      = [System.IO.Path]::GetTempFileName() -replace '\.tmp$', '.sql'
$precheckScript = [System.IO.Path]::GetTempFileName() -replace '\.tmp$', '.sql'
try {
    [System.IO.File]::WriteAllText($sqlScript,      (Extract-Block 'CHECK_APL_SQL'),  [System.Text.Encoding]::UTF8)
    [System.IO.File]::WriteAllText($precheckScript, (Extract-Block 'PRECHECK_SQL'),   [System.Text.Encoding]::UTF8)
} catch {
    Write-Error "Failed to extract embedded SQL: $_"
    exit 1
}

# --------------------------------------------------------------------------
# Interactive prompts for missing mandatory parameters
# --------------------------------------------------------------------------
if (-not $HanaHost)         { $HanaHost        = Read-Host "TENANT DB host:port (default: $DefaultHost)" ; if (-not $HanaHost)        { $HanaHost        = $DefaultHost             } }
if (-not $User)             { $User            = Read-Host "TENANT DB system user (default: $DefaultUser)" ; if (-not $User)            { $User            = $DefaultUser             } }
if (-not $Password)         { $Password        = Read-Host "TENANT DB system user password (default: $DefaultPassword)" ; if (-not $Password)        { $Password        = $DefaultPassword         } }
if (-not $CheckAplPassword) { $CheckAplPassword= Read-Host "CHECK_APL user password, mind password policy (default: $DefaultCheckAplPassword)" ; if (-not $CheckAplPassword) { $CheckAplPassword = $DefaultCheckAplPassword } }
if (-not $Format)           { $Format          = $DefaultFormat      }
if (-not $SignalError)      { $SignalError      = $DefaultSignalError }
if (-not $UseSsl)           { $UseSsl          = $DefaultUseSsl      }
if (-not $OutputFile)       { $OutputFile      = Read-Host "Output file path (use 'stdout' for console, default: $DefaultOutputFile)" ; if (-not $OutputFile) { $OutputFile = $DefaultOutputFile } }

# --------------------------------------------------------------------------
# Find hdbsql.exe
# --------------------------------------------------------------------------
$hdbsql = $null
$hdbsql = Get-Command hdbsql.exe -ErrorAction SilentlyContinue | Select-Object -ExpandProperty Source
if (-not $hdbsql -and (Test-Path 'C:\Program Files\SAP\hdbclient\hdbsql.exe')) {
    $hdbsql = 'C:\Program Files\SAP\hdbclient\hdbsql.exe'
}
if (-not $hdbsql) {
    $candidates = Get-ChildItem 'C:\Program Files\SAP' -Filter 'hdbclient*' -Directory -ErrorAction SilentlyContinue |
                  Sort-Object Name -Descending
    foreach ($c in $candidates) {
        $candidate = Join-Path $c.FullName 'hdbsql.exe'
        if (Test-Path $candidate) { $hdbsql = $candidate; break }
    }
}
if (-not $hdbsql) {
    Write-Error "hdbsql.exe not found in PATH or C:\Program Files\SAP\hdbclient*"
    Remove-Item $sqlScript, $precheckScript -Force -ErrorAction SilentlyContinue
    exit 1
}

Write-Host "Checking SAP HANA instance $HanaHost as $User to $OutputFile ..."
Write-Host ""
...
# --------------------------------------------------------------------------
$sslOpts = @()
if ($UseSsl -eq 'on') { $sslOpts = @('-e', '-ssltrustcert') }

$commonArgs = @('-n', $HanaHost) + $sslOpts + @('-u', $User, '-p', $Password)

$varString     = "OUTPUT_FORMAT=$Format,SIGNAL_ERROR=$SignalError,SYSTEM_USER=$User,SYSTEM_PASSWORD=$Password,CHECK_APL_PASSWORD=$CheckAplPassword"
$preVarString  = "SYSTEM_USER=$User,SYSTEM_PASSWORD=$Password,CHECK_APL_PASSWORD=$CheckAplPassword"

$mainArgs     = $commonArgs + @('-V', $varString,    '-j', '-I', $sqlScript,      '-A', '-a', '-F', ' ') + $ExtraArgs
$precheckArgs = $commonArgs + @('-V', $preVarString, '-j', '-I', $precheckScript, '-A', '-a', '-F', ' ') + $ExtraArgs
$connArgs     = $commonArgs + @('-j', '-Q', 'SELECT 1 FROM DUMMY')

if ($ShowCmdOnly) {
    $mask = { param($s) $s -replace [regex]::Escape($Password), '****' `
                           -replace [regex]::Escape($CheckAplPassword), '****' }
    Write-Host "Precheck hdbsql command:"
    Write-Host (& $mask ("$hdbsql " + ($precheckArgs -join ' ')))
    Write-Host ""
    Write-Host "Final hdbsql command:"
    $displayMain = "$hdbsql " + ($mainArgs -join ' ')
    if ($OutputFile -ne 'stdout') { $displayMain += " > $OutputFile" }
    Write-Host (& $mask $displayMain)
    Remove-Item $sqlScript, $precheckScript -Force -ErrorAction SilentlyContinue
    exit 0
}
...
# Connection check
# --------------------------------------------------------------------------
Write-Host "Running pre-check ..."
& $hdbsql @connArgs 2>&1 | Out-Null
if ($LASTEXITCODE -ne 0) {
    Write-Error "Unable to connect to HANA instance $HanaHost as user $User with the provided password (hdbsql exit code 3 - authentication error)"
    Remove-Item $sqlScript, $precheckScript -Force -ErrorAction SilentlyContinue
    exit 3
}
Write-Host "Connection to HANA instance $HanaHost as user $User successful."

# --------------------------------------------------------------------------
# Pre-check SQL
# --------------------------------------------------------------------------
$preOut = & $hdbsql @precheckArgs 2>&1
if ($preOut -match '10001') {
    Write-Error "Fatal Error: pre-check detected $User user does not have 'USER ADMIN' privilege"
    Remove-Item $sqlScript, $precheckScript -Force -ErrorAction SilentlyContinue
    exit 10001
}
if ($preOut -match '10002') {
    Write-Error "Fatal Error: pre-check detected provided password for temporary CHECK_APL user does not match current password policy"
    Remove-Item $sqlScript, $precheckScript -Force -ErrorAction SilentlyContinue
    exit 10002
}
Write-Host "Pre-check completed successfully. Actual check can be done"

# --------------------------------------------------------------------------
# Main check
# --------------------------------------------------------------------------
$output = & $hdbsql @mainArgs 2>&1 | Where-Object { $_ -notmatch 'HY000' }
...
Remove-Item $sqlScript, $precheckScript -Force -ErrorAction SilentlyContinue
Write-Host "Check completed with exit code $exitCode."
exit $exitCode

# ==============================================================================
# Embedded SQL data — extracted at runtime by Extract-Block above.
# Everything below this line is pure data; wrapped in a block comment so
# PowerShell does not attempt to parse the SQL syntax.
# ==============================================================================
<#
#___CHECK_APL_SQL_START___
-- ================================================================
...
--
-- Options:
--  -h, --host <host:port>              TENANT DB host and port (default: hana:30015)
--  -u, --user <user>                   TENANT DB system user (default: SYSTEM)
--  -p, --password <password>           TENANT DB user password (default: Manager1)
...
BEGIN
    ...
    DECLARE scriptserver_port NVARCHAR(100);
	...
	SELECT "SQL_PORT" into scriptserver_port FROM "M_SERVICES" WHERE "SERVICE_NAME"='scriptserver' AND "HOST"=:hostname;
	...
	:results.insert(('DANGER ZONE : Remove script server','from database ' || :database_name,:hana_shared_sid || '/hdbclient/hdbsql -i ' || :instance_number || ' -d SYSTEMDB -u SYSTEM "ALTER DATABASE ' || :database_name || ' REMOVE ''scriptserver'' AT ''' || :hostname ||':'||:scriptserver_port||'''"'));
...
BEGIN
	...
	:results.insert(('Remove script server','from database ' || :database_name,'/usr/sap/'||:database_name||'/HDB'||:instance_number||'/exe/hdbsql -i ' || :instance_number || ' -d SYSTEMDB -u SYSTEM "ALTER DATABASE ' || :database_name || ' REMOVE ''scriptserver'' AT ''' || :hostname ||'''"'));
...
#___CHECK_APL_SQL_END___
#___PRECHECK_SQL_START___
-- @configSQL
DO BEGIN
