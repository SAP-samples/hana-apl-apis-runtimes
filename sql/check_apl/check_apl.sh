...
# Self-heal if this script was mangled to CRLF line endings (e.g. Windows transit).
# The shebang above is already unusable in that case, so this branch is only reached
# when the user invokes us as `bash check_apl.sh`. Re-exec a scrubbed copy of ourselves
# so every downstream line (sed anchors, heredoc sentinel, quoting) sees pure LF.
if grep -q $'\r' "$0" 2>/dev/null; then
    _healed="$(mktemp --suffix=.sh)"
    tr -d '\r' < "$0" > "$_healed"
    chmod 755 "$_healed"
    exec bash "$_healed" "$@"
fi
...
Options:
  -h, --host <host:port>                TENANT DB host and port (default: $DEFAULT_DB_HOST)
  -u, --system_user <user>              TENANT  DB system user (default: $DEFAULT_DB_SYSTEM)
  -p, --system_password <password>      TENANT DB system user password (default: $DEFAULT_DB_SYSTEM_PASSWORD)
  ...
  --help                                Show this help message and exit
When missing, the script will prompt for mandatory parameters (tenant host,user,password) interactively.
Extra hdbsql parameters can be passed after the mandatory ones, and they will be appended to the hdbsql command.
...
# Interactive values for missing parameters host,system user, system user password for TENANT DB
if [[ -z "$DB_HOST" ]]; then
    read -rp "Enter TENANT DB host:port [$DEFAULT_DB_HOST]: " DB_HOST
    DB_HOST=${DB_HOST:-$DEFAULT_DB_HOST}
fi
if [[ -z "$DB_SYSTEM" ]]; then
    read -rp "Enter TENANT DB system user [$DEFAULT_DB_SYSTEM]: " DB_SYSTEM
    DB_SYSTEM=${DB_SYSTEM:-$DEFAULT_DB_SYSTEM}
fi
if [[ -z "$DB_SYSTEM_PASSWORD" ]]; then
    read -rsp "Enter TENANT DB's system user password [$DEFAULT_DB_SYSTEM_PASSWORD]: " DB_SYSTEM_PASSWORD
    DB_SYSTEM_PASSWORD=${DB_SYSTEM_PASSWORD:-$DEFAULT_DB_SYSTEM_PASSWORD}
    echo
fi
if [[ -z "$CHECK_APL_PASSWORD" ]]; then
    read -rsp "Enter CHECK_APL user password; Do not forget to match your current password policy! [$DEFAULT_CHECK_APL_PASSWORD]: " CHECK_APL_PASSWORD
...
# Extract embedded SQL blocks from this script to temp files.
# The [[:space:]]*$ anchor tolerates a stray trailing \r on the marker lines
# (Windows CRLF corruption); tr -d '\r' then scrubs any CR from the payload itself.
_extract_block() {
    sed -n "/^###${1}_START###[[:space:]]*$/,/^###${1}_END###[[:space:]]*$/{/^###${1}_START###[[:space:]]*$/d;/^###${1}_END###[[:space:]]*$/d;p}" "$0" | tr -d '\r'
}
SQL_SCRIPT="$(mktemp --suffix=.sql)"
_extract_block "CHECK_APL_SQL" > "$SQL_SCRIPT"
PRECHECK_SQL_SCRIPT="$(mktemp --suffix=.sql)"
_extract_block "PRECHECK_SQL" > "$PRECHECK_SQL_SCRIPT"
trap 'rm -f "$SQL_SCRIPT" "$PRECHECK_SQL_SCRIPT"' EXIT
...
# ── Embedded SQL data ──────────────────────────────────────────────────────────
# Everything below this line is data extracted at runtime by _extract_block().
# Wrapped in a null heredoc so bash does not attempt to parse it as shell code.
# sed reads the raw file lines regardless of heredoc semantics, so extraction works.
: <<'###END_OF_SHELL_CODE###'
###CHECK_APL_SQL_START###
-- ================================================================
