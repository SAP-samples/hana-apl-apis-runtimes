...
-- The value of the target key is set, but since the model has a unique target, the appropriate target variable is automatically used, and there's no need to use the extended OPERATION_CONFIG_DETAILED_T.
-- 
...
insert into CREATE_AND_TRAIN_CONFIG values ('APL/TargetKey', '0');
